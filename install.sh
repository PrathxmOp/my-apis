...
check_out_dir() {
  ...
  # If output_dir is '.'or '/opt', create inside it `blockguard` directory
...
unpack() {
  ...
  dir_name=$(echo "${pkg_name}" | sed -E -e 's/(.*)(\.tar\.gz|\.zip)//')
  ...
  if [ -L "/usr/local/bin/${exe_name}" ] &&       [ "$(readlink -f "/usr/local/bin/${exe_name}")" = "$(readlink -f "${output_dir}/${exe_name}")" ]; then
    symlink_exists='1'
  fi
...
download() {
  ...
  log "Downloading BlockGuard CLI package: $url"
